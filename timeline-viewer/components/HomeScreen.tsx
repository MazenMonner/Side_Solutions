...
const TimelineCard: React.FC<{ timeline: TTimeline; onClick: () => void }> = ({ timeline, onClick }) => (
  <button
    onClick={onClick}
    className="w-full text-left bg-slate-800 p-6 rounded-lg border border-slate-700 hover:border-indigo-500 hover:bg-slate-700/50 transition-all duration-300 ease-in-out shadow-lg"
  >
    <h2 className="text-xl font-bold text-white">{timeline.name}</h2>
    <p className="text-slate-400 mt-2 text-sm">{timeline.description}</p>
